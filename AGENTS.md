...
* **为什么不并行？** 单核向量化在内存带宽成为瓶颈前即可满足本工具定位的典型规模（\~10⁷ 级行）。
* **季节定义能改吗？** 需求已明确“默认即可”。`pandas` 的季度已覆盖北半球常规 DJF/MAM/JJA/SON，因此无需扩展。
* **CLI 未来会不会加入？** 当前不在范围内；若确有场景，可另立模块，不得污染核心包。

---

> **即刻动手**：依循本手册完成 *resan* 的 MVP。实现后确保通过单测与静态检查，再进行后续迭代。
